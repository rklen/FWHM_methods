...
f<-function(x){N*(k[2]-k[1])/sqrt(2*pi)*exp(-1/2*x^2)}
y<-hist(z,breaks=k,plot=F)$counts
(k[2]-k[1])*N/sqrt(2*pi)
sqrt(2*log(2))
hist(z,breaks=k,plot=T,col='white',xlim=c(-3.5,3.5),ylim=c(0,330))
par(new=TRUE)
plot(f,col='blue',xlim=c(-3.5,3.5),ylim=c(0,330),xlab='',ylab='')
points(c(0+0i,0+f(0)*1i),type='l',col='blue')
points(c(-sqrt(2*log(2))+f(0)*1i/2,sqrt(2*log(2))+f(0)*1i/2),
       type='l',col='blue')
...
#figure 1a
cols=c('black','lightblue','steelblue','blue','gray','green','purple')
pSty<-c(1,2,3,4,5,6,8)
lSty<-c(3,2,1,3,2,1,3)
y1<-read.csv('y001.csv')
plot(c(3,3.5,4,4.5,5),y1[,6],xlim=c(3,5),ylim=c(0,0.6),type='l',lty=lSty[1],col=cols[1],ylab='',
     xlab='log_10(N)',cex.axis=1.5,cex.lab=2,lwd=2)
points(c(3,3.5,4,4.5,5),y1[,6],pch=pSty[1],col=cols[1],lwd=2)
for(i in c(2:7)){
  par(new=TRUE)
  plot(c(3,3.5,4,4.5,5),y1[,c(1:5,8,9)[i]],xlim=c(3,5),ylim=c(0,0.6),type='l',
       axes=F,xlab='',ylab='',lty=lSty[i],col=cols[i],lwd=2)
  points(c(3,3.5,4,4.5,5),y1[,c(1:5,8,9)[i]],pch=pSty[i],col=cols[i],lwd=2)
}
legend('topright',
       legend=c('F1','F2','F3','F4','F5','F6','F7'),
       lty=lSty,pch=pSty,col=cols,cex=1.6,lwd=rep(2,7))

#figure 1b
y2<-read.csv('y002.csv')
plot(c(1,1.5,2,2.5,3,3.5,4),y2[,6],xlim=c(1,4),ylim=c(0,1),type='l',lty=lSty[1],
     col=cols[1],ylab='',xlab='log_10(n)',cex.axis=1.5,cex.lab=2,
     lwd=2)
points(c(1,1.5,2,2.5,3,3.5,4),y2[,6],pch=pSty[1],col=cols[1],lwd=2)
for(i in c(2:7)){
  par(new=TRUE)
  plot(c(1,1.5,2,2.5,3,3.5,4),y2[,c(1:5,8,9)[i]],xlim=c(1,4),ylim=c(0,1),type='l',
       axes=F,xlab='',ylab='',lty=lSty[i],col=cols[i],lwd=2)
  points(c(1,1.5,2,2.5,3,3.5,4),y2[,c(1:5,8,9)[i]],pch=pSty[i],col=cols[i],lwd=2)
}
#legend('topleft',legend=c('F1','F2','F3','F4','F5','F6','F7','F8','F9'),pch=8,col=cols,cex=1.5)

#figure 1c
y3<-read.csv('y003.csv')
plot(0.1*c(0:10),y3[,6],xlim=c(0,1),ylim=c(0,0.07),type='l',
     lty=lSty[1],col=cols[1],ylab='',xlab='s',cex.axis=1.5,cex.lab=2,
     lwd=2)
points(0.1*c(0:10),y3[,6],pch=pSty[1],col=cols[1],lwd=2)
for(i in c(2:7)){
  par(new=TRUE)
  plot(0.1*c(0:10),y3[,c(1:5,8,9)[i]],xlim=c(0,1),ylim=c(0,0.07),type='l',
       axes=F,xlab='',ylab='',lty=lSty[i],col=cols[i],lwd=2)
  points(0.1*c(0:10),y3[,c(1:5,8,9)[i]],pch=pSty[i],col=cols[i],lwd=2)
}
#legend('topleft',legend=c('F1','F2','F3','F4','F5','F6','F7','F8','F9'),pch=8,col=cols,cex=1.5)

#figure 1d
y4<-read.csv('y004.csv')
plot(10*c(0:10),y4[,6],xlim=c(0,100),ylim=c(0,0.55),type='l',
     lty=lSty[1],col=cols[1],ylab='',xlab='t',cex.axis=1.5,cex.lab=2,
     lwd=2)
points(10*c(0:10),y4[,6],pch=pSty[1],col=cols[1],lwd=2)
for(i in c(1:6,8,9)){
  par(new=TRUE)
  plot(10*c(0:10),y4[,c(1:5,8,9)[i]],xlim=c(0,100),ylim=c(0,0.55),type='l',
       axes=F,xlab='',ylab='',lty=lSty[i],col=cols[i],lwd=2)
  points(10*c(0:10),y4[,c(1:5,8,9)[i]],pch=pSty[i],col=cols[i],lwd=2)
}
#legend('topleft',legend=c('F1','F2','F3','F4','F5','F6','F7','F8','F9'),pch=8,col=cols,cex=1.5)
